...
function H.renderInventoryPanel()
  ...
  for _, item in ipairs(sorted) do
    ...
    if echoLink then
      local command = string.format("send(%q, true)", "ITEM " .. name)
      echoLink(
...
local function craftingBar(percent)
  local width = 20
  local value = math.max(0, math.min(100, tonumber(percent) or 0))
  local filled = math.floor((value / 100) * width + 0.5)
  return "[" .. string.rep("=", filled) .. string.rep("-", width - filled) .. "]"
end

function H.renderCrafting()
  if not H.modernBuilt or not H.onboarding then return false end
...
function H.renderOnboarding()
  if not H.modernBuilt then return end
  if H.renderCrafting() then return end
...
function H.onCraftingModern()
  local data = gmcp and gmcp.Dreams and gmcp.Dreams.Crafting
  if not data then return end
  H.state.crafting = data
  H.craftingNoticeSerial = (tonumber(H.craftingNoticeSerial) or 0) + 1
  local serial = H.craftingNoticeSerial
  ...
  H.onboarding:echo(message)
  setTooltip(H.onboarding, "")
  if tempTimer then
    tempTimer(1.2, function()
      if H.craftingNoticeSerial == serial and not ((H.state.crafting or {}).active) then
        H.renderOnboarding()
      end
    end)
  end
...
function H.registerModernHandlers()
  local names = {
    ...
    crafting = {"gmcp.Dreams.Crafting", H.onCraftingModern},
