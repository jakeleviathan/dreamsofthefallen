...
class LivingWorldTests(unittest.TestCase):
    ...
    def test_return_mail_reports_change_without_creating_fomo_reward(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertNotIn("missed reward", row["body"].lower())
            self.assertNotIn("nothing in this letter", row["body"].lower())
            self.assertNotIn("penalty", row["body"].lower())
            self.assertIn("current word from the road", row["body"].lower())
