...
COMMANDS: tuple[CommandEntry, ...] = (
    ...
    CommandEntry("character", "ABILITIES ALL", "Show the complete authored class progression, including future level unlocks."),
    CommandEntry("character", "ABILITY <name>", "Inspect one ability's command, cost, cooldown, mastery progress, current bonuses, and next mastery milestone."),
    ...
    CommandEntry("economy", "MINING / HARVESTING / HERBALISM", "Inspect that gathering skill, its progression tier, next milestone, and gatherables in the current room."),
    CommandEntry("economy", "FORGE [item]", "Open the local Blacksmithing workshop or forge a named Blacksmithing recipe."),
    ...
    CommandEntry("economy", "POTIONS / DRINK <potion>", "List carried alchemical drinks or consume one for its restorative and temporary effects."),
    CommandEntry("economy", "PERFUMERY / RECIPES PERFUME", "Browse the Alchemy perfume branch, including tiered XP bonuses, ingredients, and trivial values."),
    CommandEntry("economy", "RECIPES SUMMARY / RECIPE COUNTS", "Show the number of currently known recipes in each crafting profession; undiscovered secrets remain hidden."),
    ...
    CommandEntry("economy", "CRAFT <recipe>", "Craft by item/recipe name at the required station using real inventory materials."),
    CommandEntry("economy", "SOLS / COINS / MONEY", "Show your sun-stamped Sol balance as flames, embers, and sparks."),
    CommandEntry("economy", "SHOP / LIST / WARES", "Show nearby merchant stock with prices in Sols."),
    CommandEntry("economy", "BUY [qty] <item>", "Buy one or more items from a nearby merchant using Sols."),
    CommandEntry("economy", "SELL [qty] <item>", "Sell carried, unequipped items to a nearby merchant for 35% of baseline value."),
    CommandEntry("economy", "VALUE <item>", "Ask a nearby merchant what they would pay for a carried item."),
    ...
    CommandEntry("world", "MAIL / POST / INBOX", "List persistent world and player letters; unread messages are marked with *."),
    CommandEntry("world", "MAIL READ <number> / READ MAIL <number>", "Read one visible letter and mark it read."),
    CommandEntry("social", "MAIL SEND [TO] <player>", "Compose persistent mail for an online or offline character."),
    CommandEntry("world", "MAIL DELETE <number>", "Soft-delete one letter from your inbox."),
    CommandEntry("world", "MAIL CLEAR READ / MAIL CLEAR ALL", "Clear read mail or the whole inbox after confirmation."),
    ...
    CommandEntry("veyra", "SELL DEMAND <qty> <item>", "Sell complete demand bundles for Sols."),
