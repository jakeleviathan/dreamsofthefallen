...
def _inventory_snapshot(session) -> dict:
    from mud.item_heritage import heritage_summary_data, is_special_item
    ...
    for row in session.database.list_items(character.id):
        ...
        heritage = heritage_summary_data(session.database, character.id, item_key)
        items.append(
            {
                "key": item_key,
                ...
                "heritage": heritage if heritage["tracked"] or is_special_item(item_key) else None,
            }
        )
